 ...
 * @Author LiuLiWei
 */
public abstract class RxBasePermissionFragment<V extends RxBaseView, P extends RxBasePresenter<V>> extends RxBaseFragment<V,P> implements ActivityCompat.OnRequestPermissionsResultCallback {
    /**
    ...
    protected void sendMultiPermissionRequests(@NonNull String permissionArr[]) {
        ActivityCompat.requestPermissions(getActivity(), permissionArr, PERMISSIONS_REQUEST_CODE);
    }
    ...
    protected void showPermissionDialog(String title, String content,
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(getRxContext());
    ...
    protected boolean dealWithPermission(@NonNull String permission, String tipContent) {
        if(!checkPermission(permission)) {
            if(shouldShowPermissionRationale(permission)) {
                new MaterialDialog.Builder(getRxContext())
                        .title("提示")
                        .content(tipContent)
                        .positiveText("确定")
                        .negativeText("取消")
