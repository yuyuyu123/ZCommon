...
import com.cc.android.zcommon.model.BaseModel;
 ...
 * @Author:LiuLiWei
 */
public final class ZDbDataHelper {
    ...
    public ArrayList<BaseModel> select(final String table, final String selection, final String[] selectionArgs,
        ...
        synchronized (mDbManager) {
            try {
                database = mDbManager.getReadableDatabase();
    ...
    private BaseModel changeToModule(Cursor cursor, Class<?> moduleClass)
        ...
        synchronized (mDbManager) {
            ...
            String filedValue;
            int filedIntValue = -1;
            long filedLongValue = -1;
            double filedDoubleValue = -1d;
            ...
            for (int j = 0; j < columnsCount; j++) {
                ...
                if (field.get(module) instanceof Integer) {
                    filedIntValue = cursor.getInt(j);
                    if (filedIntValue != -1) {
                        field.set(module, filedIntValue);
                    }
                } else if (field.get(module) instanceof Long) {
                    filedLongValue = cursor.getLong(j);
                    if (filedLongValue != -1) {
                        field.set(module, filedLongValue);
                    }
                } else if (field.get(module) instanceof Double) {
                    filedDoubleValue = cursor.getDouble(j);
                    if (filedDoubleValue != -1) {
                        field.set(module, filedDoubleValue);
                    }
                } else {
    ...
    public long insert(final String table, final BaseModel module) {
        synchronized (mDbManager) {
            ...
            return mDbManager.insert(table, null, values);
        }
    }
     ...
     * @return
     */
    public boolean insert(final String table, final ArrayList<BaseModel> modules) {
        synchronized (mDbManager) {
    ...
    public int delete(final String table, final String whereClause, final String[] whereArgs) {
        synchronized (mDbManager) {
            return mDbManager.delete(table, whereClause, whereArgs);
        }
    }
    ...
    public long update(final String table, final String whereClause, final String[] whereArgs, final BaseModel module) {
        synchronized (mDbManager) {
            ContentValues values = moduleToContentValues(module);
            return mDbManager.update(table, values, whereClause, whereArgs);
        }
    }
    ...
    private ContentValues moduleToContentValues(final BaseModel module) {
        ...
        long fieldValueForLong = -1;
        double fieldValueForDouble = -1d;
        try {
            for (Field field : fields) {
                fieldName = field.getName();
                if (TextUtils.isEmpty(fieldName) || fieldName.equals("$change") || fieldName.equals("serialVersionUID")) {
                    continue;
                }
                if (field.get(module) instanceof String) {
                    ...
                    fieldValueForLong = (Long) field.get(module);
                    if (fieldValueForLong != -1) {
                        values.put(fieldName, fieldValueForLong);
                    }
                } else if (field.get(module) instanceof Double) {
                    fieldValueForDouble = (Double) field.get(module);
                    if (fieldValueForDouble != -1) {
                        values.put(fieldName, fieldValueForDouble);
                    }
                }
            }
